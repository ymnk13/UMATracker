...
Blockly.Python['cv_erosion'] = function(block) {
    ...
    var code = [];
    var kernel_name = "self.kernel_{0}".format(orgFunc.randStr());
    code.push('#{0} = np.ones(({1},{1}),np.uint8)\n'.format(kernel_name, text_size));
...
Blockly.Python['cv_dilation'] = function(block) {
    ...
    var kernel_name = "self.kernel_{0}".format(orgFunc.randStr());
    code.push('#{0} = np.ones(({1},{1}),np.uint8)\n'.format(kernel_name, text_size));
    code.push( Blockly.Python.joinCodesToOperator(value_name,'','cv2.dilate({0},{1},iterations = 1)\n'.format('{0}',kernel_name)));
    var str_code =code.join("\n");
    // TODO: Change ORDER_NONE to the correct strength.
    return [str_code, Blockly.Python.ORDER_MEMBER];
};
...
Blockly.Python['cv_opening'] = function(block) {
    ...
    var kernel_name = "self.kernel_{0}".format(orgFunc.randStr());
    code.push('#{0} = np.ones(({1},{1}),np.uint8)\n'.format(kernel_name, text_size));
    code.push( Blockly.Python.joinCodesToOperator(value_name,'','cv2.morphologyEx({0},cv2.MORPH_OPEN,{1})\n'.format('{0}',kernel_name)));
    var str_code =code.join("\n");
    // TODO: Change ORDER_NONE to the correct strength.
    return [str_code, Blockly.Python.ORDER_MEMBER];
};
...
Blockly.Python['cv_closing'] = function(block) {
    ...
    var kernel_name = "self.kernel_{0}".format(orgFunc.randStr());
    code.push('#{0} = np.ones(({1},{1}),np.uint8)\n'.format(kernel_name, text_size));
    code.push( Blockly.Python.joinCodesToOperator(value_name,'','cv2.morphologyEx({0},cv2.MORPH_CLOSE,{1})\n'.format('{0}',kernel_name)));
    var str_code =code.join("\n");
    // TODO: Change ORDER_NONE to the correct strength.
    return [str_code, Blockly.Python.ORDER_MEMBER];
};
