...
from PyQt5.QtWidgets import QGraphicsScene, QGraphicsView, QGraphicsPixmapItem, QFrame, QFileDialog, QGraphicsEllipseItem
#from PyQt5.QtGui import QPixmap, QImage, QTransform, QColor
from PyQt5.QtWidgets import QRubberBand,QGraphicsRectItem,QGraphicsObject
...
class EllipseForAreaSelection(QGraphicsObject):
    ...
    def mouseMoveEvent(self,event):
        ...
        if self.mouseIsPressed:
            if self.mousePressArea == 'topRect':
                self._rect.setTop(self.rectPress.y()-(self.mousePressPos.y()-mouseMovePos.y()))
            elif self.mousePressArea == 'bottomRect':
                self._rect.setBottom(self.rectPress.bottom()-(self.mousePressPos.y()-mouseMovePos.y()))
            elif self.mousePressArea == 'leftRect':
                self._rect.setLeft(self.rectPress.left()-(self.mousePressPos.x()-mouseMovePos.x()))
            elif self.mousePressArea == 'rightRect':
                self._rect.setRight(self.rectPress.right()-(self.mousePressPos.x()-mouseMovePos.x()))
            elif self.mousePressArea=='topleftRect':
                self._rect.setTopLeft(self.rectPress.topLeft()-(self.mousePressPos-mouseMovePos))
            elif  self.mousePressArea=='toprightRect':
                self._rect.setTopRight(self.rectPress.topRight()-(self.mousePressPos-mouseMovePos))
            elif  self.mousePressArea=='bottomleftRect':
                self._rect.setBottomLeft(self.rectPress.bottomLeft()-(self.mousePressPos-mouseMovePos))
            elif  self.mousePressArea=='bottomrightRect':
                self._rect.setBottomRight(self.rectPress.bottomRight()-(self.mousePressPos-mouseMovePos))
            else:
                self._rect.moveCenter(self.rectPress.center()-(self.mousePressPos-mouseMovePos))
        self.updateResizeHandles()
        self.prepareGeometryChange()

    def updateResizeHandles(self):
        self.resizeHandleSize = 4.0
        self.offset = self.resizeHandleSize*(
            self._view.mapToScene(1,0).x()
            -self._view.mapToScene(0,1).x())
